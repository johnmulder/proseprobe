...
def test_profiles_cover_non_experimental_rules_without_unknown_rules() -> None:
    ...
    assert {"S029", "T013", "V015", "V017"} == EXPERIMENTAL_RULES
...
def test_profiles_include_only_their_specialized_rules() -> None:
    ...
    assert "C007" in PROFILES["technical-docs"].rules
    assert "C008" in PROFILES["technical-docs"].rules
    assert "G016" in PROFILES["technical-docs"].rules
    assert "G017" in PROFILES["technical-docs"].rules
    assert "G019" in PROFILES["technical-docs"].rules
    assert "G022" in PROFILES["technical-docs"].rules
    assert "G024" in PROFILES["technical-docs"].rules
    assert "G025" in PROFILES["technical-docs"].rules
    assert "G029" in PROFILES["technical-docs"].rules
    assert "G031" in PROFILES["technical-docs"].rules
    assert "G037" in PROFILES["technical-docs"].rules
    ...
    assert "S022" in PROFILES["technical-docs"].rules
    assert "S025" in PROFILES["technical-docs"].rules
    assert "S028" in PROFILES["technical-docs"].rules
    assert "T010" in PROFILES["technical-docs"].rules
    assert "T012" in PROFILES["technical-docs"].rules
    assert "T014" in PROFILES["technical-docs"].rules
    assert "T015" in PROFILES["technical-docs"].rules
    assert "T016" in PROFILES["technical-docs"].rules
    assert "V009" in PROFILES["technical-docs"].rules
    assert "V010" in PROFILES["technical-docs"].rules
    assert "V011" in PROFILES["technical-docs"].rules
    assert "V013" in PROFILES["technical-docs"].rules
    assert "V014" in PROFILES["technical-docs"].rules
    assert "V016" in PROFILES["technical-docs"].rules

    assert "G011" not in PROFILES["journalism"].rules
    assert "C007" not in PROFILES["general"].rules
    assert "C008" not in PROFILES["general"].rules
    assert "G016" not in PROFILES["general"].rules
    assert "G017" not in PROFILES["general"].rules
    assert "G019" not in PROFILES["general"].rules
    assert "G022" not in PROFILES["general"].rules
    assert "G024" not in PROFILES["general"].rules
    assert "G025" not in PROFILES["general"].rules
    assert "G029" not in PROFILES["general"].rules
    assert "G031" not in PROFILES["general"].rules
    assert "G037" not in PROFILES["general"].rules
    ...
    assert "S022" not in PROFILES["general"].rules
    assert "S025" not in PROFILES["general"].rules
    assert "S028" not in PROFILES["general"].rules
    assert "T010" not in PROFILES["general"].rules
    assert "T012" not in PROFILES["general"].rules
    assert "T014" not in PROFILES["general"].rules
    assert "T015" not in PROFILES["general"].rules
    assert "T016" not in PROFILES["general"].rules
    assert "V009" not in PROFILES["general"].rules
    assert "V010" not in PROFILES["general"].rules
    assert "V011" not in PROFILES["general"].rules
    assert "V013" not in PROFILES["general"].rules
    assert "V014" not in PROFILES["general"].rules
...
def test_reverse_profile_tags_are_sorted() -> None:
    ...
    assert profile_names_for_rule("G019") == ("technical-docs",)
    assert profile_names_for_rule("G022") == ("technical-docs",)
    assert profile_names_for_rule("G024") == ("technical-docs",)
    assert profile_names_for_rule("G025") == ("technical-docs",)
    assert profile_names_for_rule("G029") == ("technical-docs",)
    assert profile_names_for_rule("G031") == ("technical-docs",)
    assert profile_names_for_rule("G037") == ("technical-docs",)
    assert profile_names_for_rule("M009") == ("technical-docs",)
    assert profile_names_for_rule("M010") == ("technical-docs",)
    assert profile_names_for_rule("S022") == ("technical-docs",)
    assert profile_names_for_rule("S025") == ("technical-docs",)
    assert profile_names_for_rule("S028") == ("technical-docs",)
    assert profile_names_for_rule("S029") == ()
    assert profile_names_for_rule("T010") == ("technical-docs",)
    assert profile_names_for_rule("T012") == ("technical-docs",)
    assert profile_names_for_rule("T013") == ()
    assert profile_names_for_rule("T014") == ("technical-docs",)
    assert profile_names_for_rule("T015") == ("technical-docs",)
    assert profile_names_for_rule("T016") == ("technical-docs",)
    ...
    assert profile_names_for_rule("V017") == ()
