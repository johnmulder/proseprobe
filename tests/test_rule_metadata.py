...
def test_metadata_reports_low_confidence_rule_defaults() -> None:
    assert get_rule_metadata_by_id("C006").default_confidence is Confidence.LOW  # type: ignore[union-attr]
    ...
    assert get_rule_metadata_by_id("V017").default_confidence is Confidence.LOW  # type: ignore[union-attr]
    ...
    assert get_rule_metadata_by_id("G022").default_confidence is Confidence.MEDIUM  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("G024").default_confidence is Confidence.MEDIUM  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("G025").default_confidence is Confidence.MEDIUM  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("G029").default_confidence is Confidence.HIGH  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("G031").default_confidence is Confidence.MEDIUM  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("G037").default_confidence is Confidence.MEDIUM  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("G038").default_confidence is Confidence.LOW  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("T010").default_confidence is Confidence.HIGH  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("T012").default_confidence is Confidence.MEDIUM  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("T013").default_confidence is Confidence.LOW  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("T014").default_confidence is Confidence.MEDIUM  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("T015").default_confidence is Confidence.HIGH  # type: ignore[union-attr]
    assert get_rule_metadata_by_id("T016").default_confidence is Confidence.MEDIUM  # type: ignore[union-attr]
...
def test_c007_metadata_matches_the_docstring_signature_contract() -> None:
    metadata = get_rule_metadata_by_id("C007")
...
def test_c006_metadata_matches_the_comment_restatement_contract() -> None:
    metadata = get_rule_metadata_by_id("C006")

    assert metadata is not None
    assert metadata.name == "Comment Restates Code"
    assert metadata.default_severity is Severity.INFO
...
def test_s028_metadata_matches_the_heading_depth_contract() -> None:
    metadata = get_rule_metadata_by_id("S028")

    assert metadata is not None
    assert metadata.name == "Excessive Heading Depth"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown",)
    assert metadata.content_scope == "raw"
...
def test_v014_metadata_matches_the_imprecise_quantity_contract() -> None:
    metadata = get_rule_metadata_by_id("V014")

    assert metadata is not None
    assert metadata.name == "Imprecise Quantity"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_v017_metadata_matches_the_needless_intensifier_contract() -> None:
    metadata = get_rule_metadata_by_id("V017")

    assert metadata is not None
    assert metadata.name == "Needless Intensifier"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.LOW
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_g016_metadata_matches_the_existential_opener_contract() -> None:
    metadata = get_rule_metadata_by_id("G016")
...
def test_g022_metadata_matches_the_former_latter_contract() -> None:
    metadata = get_rule_metadata_by_id("G022")

    assert metadata is not None
    assert metadata.name == "Former/Latter Reference"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_g025_metadata_matches_the_weak_instruction_contract() -> None:
    metadata = get_rule_metadata_by_id("G025")

    assert metadata is not None
    assert metadata.name == "Weak Instruction Verb"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_g031_metadata_matches_the_clause_overload_contract() -> None:
    metadata = get_rule_metadata_by_id("G031")

    assert metadata is not None
    assert metadata.name == "Clause/Coordination Overload"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_g037_metadata_matches_the_hedged_requirement_contract() -> None:
    metadata = get_rule_metadata_by_id("G037")

    assert metadata is not None
    assert metadata.name == "Hedged Requirement"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_g038_metadata_matches_the_undefined_comparative_contract() -> None:
    metadata = get_rule_metadata_by_id("G038")

    assert metadata is not None
    assert metadata.name == "Undefined Comparative"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.LOW
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_t016_metadata_matches_the_slash_alternative_contract() -> None:
    metadata = get_rule_metadata_by_id("T016")

    assert metadata is not None
    assert metadata.name == "Slash Alternative"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_t014_metadata_matches_the_parenthetical_overload_contract() -> None:
    metadata = get_rule_metadata_by_id("T014")

    assert metadata is not None
    assert metadata.name == "Parenthetical Overload"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_t010_metadata_matches_the_punctuation_contract() -> None:
    metadata = get_rule_metadata_by_id("T010")

    assert metadata is not None
    assert metadata.name == "Repeated or Mixed Punctuation"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.HIGH
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_t012_metadata_matches_the_ellipsis_contract() -> None:
    metadata = get_rule_metadata_by_id("T012")

    assert metadata is not None
    assert metadata.name == "Rhetorical Ellipsis"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.MEDIUM
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
...
def test_t013_metadata_matches_the_all_caps_contract() -> None:
    metadata = get_rule_metadata_by_id("T013")

    assert metadata is not None
    assert metadata.name == "ALL-CAPS Emphasis"
    assert metadata.default_severity is Severity.INFO
    assert metadata.default_confidence is Confidence.LOW
    assert metadata.applies_to == ("markdown", "python")
    assert metadata.content_scope == "prose"
